...
### 평가 자산
- `data/ground_truth/{notice_id}/` — 60건 골드셋 (D5 시점 6건 부분, 추가 라벨링 진행 중)
- `data/few_shot/{필드키}.md` — 51개 (T0~T3 보강 완료)
- `tests/fixtures/gemini_cassettes/` — Gemini 응답 VCR 캐시 (CI 비용 0)
...
출력: `reports/{YYYYMMDD_HHMMSS}/summary.md` + `per_field.csv` + `failures/*.md`

### 회귀 테스트
- `pytest tests/test_regression.py::test_min_accuracy`
- 임계값 램프: D7=70% / 2주차=85% / 1개월=95%
- 정확도 측정은 골드셋 검수 완료 후 단계별 활성 (현재 placeholder)

### 매처 (`eval/matchers.py`)

| 매처 | 적용 필드군 |
|---|---|
| exact | int·bool·enum·코드 |
| numeric_tolerance | float (투찰율·지역의무비율) |
| iso_datetime_eq | 날짜 6필드 |
| set_eq | 지역(list) |
| nested_set_eq | 종목 (nested_list[str]) |
| dict_keys_subset_eq | 보험료합계·금액정보 |
| text_normalized_eq | 자유 텍스트 |
| text_semantic | 특수실적·적격평가기준_세부 (정확/부분/의미 3단계) |

### 점수 산식
- correct=1.0, partial=0.5, mismatch=0
- 전체 = 가중평균 (`required=true` 14필드 가중치 2.0, 나머지 1.0)
- 추가: Source Grounding 정확도, Hallucination Rate

### 꿈자동화 통합 (D6)
- `app/adapter_dream.py` 가 BidExtraction → 꿈자동화 ai_columns 행 리스트로 변환
- `app/routes.py` 의 `?format=dream` 분기로 노출
- 매핑 표: 꿈자동화 측 `docs/integration/dingpago_v1_mapping.md`
- 호출 규약: `POST /analyze?format=dream` (내부 호출, 인증 미설정)
